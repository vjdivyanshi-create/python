...
select region_id, count(distinct node_id) as nodes_per_region from customer_nodes group by region_id;

-- Q3 How many customers are allocated to each region?--
select region_id, count(distinct customer_id) as customer_per_region from customer_nodes group by region_id;
...
select avg(days_in_node) as Avg_days_before_reallocation
from node_duration

-- Q5 What is the median, 80th and 95th percentile for this same reallocation days metric for each region?--
with node_days as(
select r.region_name, cn.customer_id, cn.node_id,
...
select txn_type,count(*) as unique_count,sum(txn_amount)as total_amount from customer_transactions group by txn_type;
...
select
avg(deposite_count*1.0) as avg_deposite_count,
avg(deposite_amount*1.0)as avg_deposite_amount
...
WHERE prev_balance IS NOT NULL
  AND (
        (closing_balance - prev_balance) * 1.0
        / NULLIF(prev_balance, 0)
      ) > 0.05;